...
const MapComponent = ({ scrollX }) => {
  ...
  return data.map((k, i) => {
    const inputRange = [(i - 1) * imageW, i * imageW, (i + 1) * imageW];
    const opacity = scrollX.interpolate({
      inputRange,
      outputRange: [0, 1, 0]
    })
    return (
      <Animated.Image key={i} source={{ uri: k.url }} style={[StyleSheet.absoluteFillObject, { opacity }]} blurRadius={30} />
    )
  })
  ...
  return (
    <>
      ...
      <View style={{ display: "flex", flex: 1, backgroundColor: "white" }}>
        ...
        <Animated.FlatList showsHorizontalScrollIndicator={false} data={data} keyExtractor={(_, index) => { index.toString() }}
